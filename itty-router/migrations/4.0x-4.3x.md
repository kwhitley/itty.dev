# Migration - 4.0x to 4.3x

v4.3x introduces two advanced routers, [`Router`](/itty-router/routers/router) and [`AutoRouter`](/itty-router/routers/autorouter).  These introduce the concept of `before` and `after` stages (and a `catch` handler for thrown errors) to eliminate the need for chaining onto `router.fetch` Promises in order to format responses, handle CORS, etc.
...
- Added: `after` stage - an array of handlers that processes after route-matching, and after any error thrown during the `before` stage or route-matching
- Added: `catch` - a handler that will catch any thrown error.  Errors thrown during the `after` stage will naturally not be subject to the `after` handlers (preventing it from throwing infinitely).
...
- [`json`](/itty-router/api#json) response formatter is included by default before the `after` stage, formatting any unformatted responses as JSON.
